...
# The hosts file is a template file.  Replaced the __SERVER__ field with the destination
# hostname, and create a hosts.FQDN file before running the playbook.
CURRENT_DIR=$(pwd)
#cd /opt/LinuxCOE/SCM && java -jar BuildClient.jar -log $1 $LOGGER_NAME  "Initiating Tomcat 7 deployment" Info
cd $CURRENT_DIR
sed "s/__SERVER__/${1}/" $PROJECT_DIR/hosts.template > $PROJECT_DIR/hosts.${1}
