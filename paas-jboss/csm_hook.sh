...
PROJECT_DIR=/etc/ansible/projects/paas-jboss7
...
CURRENT_DIR=$(pwd)
#cd /opt/LinuxCOE/SCM && java -jar BuildClient.jar -log $1 $LOGGER_NAME  "Initiating JBoss deployment" Info
cd $CURRENT_DIR
...
#        cd /opt/LinuxCOE/SCM && java -jar BuildClient.jar -log $1 $LOGGER_NAME "JBoss deployment complete" Pass
        exit 0
else
        echo "Provision failed"
#        cd /opt/LinuxCOE/SCM && java -jar BuildClient.jar -log $1 $LOGGER_NAME "JBoss deployment failed" Fail
        exit 1
fi
